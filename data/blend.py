...
Blend comps from both sources into ONE appraisal per slab:
  ...
  - Public marketplace listings: insured values + live asks (same item/grade) -> seed_cards_listings.json
Sold comps get added here once a data key is available.
...
for cert, c in cc.items():
    comps = [dict(x, source="marketplace_listings") for x in c["comps"]]
    ...
    cards.append(dict(name=c["name"], grader=c["grader"], certId=cert, grade=c["grade"], subjectNft=c["subjectNft"],
                      solscan=c["solscan"], sources=dict(marketplace_listings=len(comps) - n_gg, gacha_galaxy=n_gg, sold_comps=0),
                      comps=comps, appraisal=a))
    print(f"{c['name'][:48]:48} MKT={len(comps)-n_gg:2} GG={n_gg:2} -> ${a['fmvLow']/100:,.0f}-${a['fmvHigh']/100:,.0f} "
          f"pt ${a['fmvPoint']/100:,.0f} {a['confidenceTier']:6} LTV {a['ltvBps']//100}% max loan ${a['fmvLow']*a['ltvBps']/1e6:,.0f}")
json.dump(dict(source="BLEND: Gacha Galaxy oracle + public marketplace listings (sold comps pending)",
